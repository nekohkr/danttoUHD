...
#define OUTPUT_BIT_RATE 192000 / 2
 ...
 */
static int open_input_file(const std::vector<uint8_t>& input,
    ...
    AVCodecContext** input_codec_context)
{
    ...
    MemoryReader* reader = new MemoryReader(input);
    avio_buffer = static_cast<uint8_t*>(av_malloc(avio_buffer_size));
    if (!avio_buffer) return AVERROR(ENOMEM);

    avio_ctx = avio_alloc_context(avio_buffer, avio_buffer_size, 0, reader, MemoryReader::read_packet, nullptr, nullptr);
    if (!avio_ctx) {
        av_free(avio_buffer);
    ...
    *input_format_context = avformat_alloc_context();
    ...
    if ((error = avformat_open_input(input_format_context, nullptr, NULL,
        NULL)) < 0) {
        delete reader;
        fprintf(stderr, "Could not open input file (error)\n");
    ...
    if ((error = avformat_find_stream_info(*input_format_context, NULL)) < 0) {
        ...
        delete reader;
        avio_context_free(&(*input_format_context)->pb);
 ...
 * @return Error code (0 if successful)
 */
static int open_output_file(std::vector<uint8_t>& output,
    AVCodecContext* input_codec_context,
    ...
    AVCodecContext** output_codec_context)
{
    ...
    AVIOContext* avio_ctx = nullptr;
    uint8_t* avio_buffer = nullptr;
    size_t avio_buffer_size = 5000;
    const int channels = input_codec_context->ch_layout.nb_channels;

    MemoryWriter* writer = new MemoryWriter(output);
    avio_buffer = static_cast<uint8_t*>(av_malloc(avio_buffer_size));
    if (!avio_buffer) return AVERROR(ENOMEM);

    avio_ctx = avio_alloc_context(avio_buffer, avio_buffer_size, 1, writer, nullptr, MemoryWriter::write_packet, nullptr);
    if (!avio_ctx) {
        av_free(avio_buffer);
        delete writer;
        return AVERROR(ENOMEM);
    ...
    (*output_format_context)->pb = avio_ctx;

    /* Guess the desired container format based on the file extension. */
    if (!((*output_format_context)->oformat = av_guess_format(NULL, "aaa.ac3",
        NULL))) {
        fprintf(stderr, "Could not find output file format\n");
        goto cleanup;
    }

    if (!((*output_format_context)->url = av_strdup("aaa.ac3"))) {
    ...
    if (!(output_codec = avcodec_find_encoder(AV_CODEC_ID_AC3))) {
    ...
    //channels = input_codec_context->ch_layout.nb_channels;
    av_channel_layout_default(&avctx->ch_layout, channels);
    avctx->sample_rate = input_codec_context->sample_rate;
    avctx->sample_fmt = AV_SAMPLE_FMT_FLTP;
    if (channels == 6) {
        avctx->bit_rate = 640000;// OUTPUT_BIT_RATE* input_codec_context->ch_layout.nb_channels;
    }
    else if (channels == 2) {
...
cleanup:
    ...
    if (output_format_context) {
        delete static_cast<MemoryWriter*>(output_format_context->pb->opaque);
    ...
    if (input_format_context) {
        delete static_cast<MemoryWriter*>(input_format_context->pb->opaque);
